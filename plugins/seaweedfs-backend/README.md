...
- **`ownership.ts`** — two shapes, chosen per resource type, same idea as
  ...
  - IAM Groups and Policies have **no** owner/extra field at all
    ...
    Ownership is tracked in Backstage's own database instead
    (`coreServices.database`), a small `seaweedfs_ownership` table created at
    plugin init (`ensureOwnershipTable` — a plain "create if missing" check,
    not Backstage's usual migrations-file convention; a deliberate
    simplification for this one small table).
- **`seaweedfs-actions.ts`** — 8 scaffolder actions (create/delete × bucket,
  ...
  under a filer path `CanAccessPath` doesn't cover at all).
- **`managerPlugin.ts`** — list/delete routes backing the
  ...
  policies, but that's never the real access control on its own).
